...
function WorkspacePage() {
	...
	useEffect(() => {
		...
		const syncExistingWorkspaceContent = async () => {
			try {
				...
				if (!cancelled) {
					console.error("KhÃ´ng thá»ƒ Ä‘á»“ng bá»™ tráº¡ng thÃ¡i quiz/flashcard workspace:", error);
					setHasExistingWorkspaceQuiz(false);
					setHasExistingWorkspaceFlashcard(false);
				}
			}
	...
	// XÃ¡c Ä‘á»‹nh cÃ¡c chá»©c nÄƒng nÃªn disabled
	...
	// Äiá»u kiá»‡n roadmap theo thá»© tá»± 1 -> 3:
	// 1) roadmapEnabled tá»« profile pháº£i lÃ  true
	// 2) workspace pháº£i cÃ³ tÃ i liá»‡u ACTIVE
	// 3) roadmap structure khÃ´ng bá»‹ missing
	...
	// RiÃªng nÃºt roadmap á»Ÿ Studio: kiá»ƒm tra theo thá»© tá»±
	// 0) Náº¿u Ä‘Ã£ cÃ³ phase thÃ¬ luÃ´n hiá»‡n bÃ¬nh thÆ°á»ng (khÃ´ng disable)
	// 1) Pháº£i cÃ³ tÃ i liá»‡u ACTIVE
	// 2) roadmapEnabled tá»« profile pháº£i lÃ  true
	// 3) roadmap structure khÃ´ng bá»‹ missing
	...
	useEffect(() => {
		...
		if (isQuizDeepLink) {
			...
			// Cháº·n redirect theo activeView cÅ© (láº¥y tá»« sessionStorage) trÆ°á»›c khi route deep-link hydrate xong.
	...
	const handleDeleteMaterialsForProfileUpdate = useCallback(async () => {
		...
		try {
			...
			setProfileConfigOpen(true);
			navigate(`/workspace/${workspaceId}`, { replace: true });
			showSuccess("ÄÃ£ xÃ³a tÃ i liá»‡u hiá»‡n táº¡i. Báº¡n cÃ³ thá»ƒ cáº­p nháº­t onboarding ngay bÃ¢y giá».");
		} catch (error) {
			console.error("Failed to prepare workspace for onboarding update:", error);
			showError(error?.message || "KhÃ´ng thá»ƒ xÃ³a dá»¯ liá»‡u hiá»‡n táº¡i Ä‘á»ƒ cáº­p nháº­t onboarding.");
		} finally {
	...
	const handleSaveProfileConfig = useCallback(async (currentStep, data) => {
		try {
			...
			// Step 3 chá»‰ lÆ°u draft. Chá»‰ khi user Confirm má»›i khÃ³a onboarding vÃ  Ä‘Ã³ng wizard.
			showSuccess("ÄÃ£ lÆ°u cáº¥u hÃ¬nh (draft). Báº¡n cÃ³ thá»ƒ tiáº¿p tá»¥c chá»‰nh sá»­a hoáº·c báº¥m XÃ¡c nháº­n Ä‘á»ƒ hoÃ n táº¥t.");
			return savedProfile;
		} catch (error) {
		...
		try {
			...
			showSuccess(
				translateOrFallback(
					...
					"HoÃ n thÃ nh thiáº¿t láº­p workspace thÃ nh cÃ´ng."
				)
			);
			navigateToWorkspaceRoot();
		} catch (error) {
			console.error("Failed to confirm profile:", error);
			showError(error?.message || "KhÃ´ng thá»ƒ xÃ¡c nháº­n onboarding. Vui lÃ²ng thá»­ láº¡i.");
			throw error;
		}
	}, [
		closeProfileDialogs,
	...
	// Cháº·n back navigation quay láº¡i mÃ n hÃ¬nh onboarding sau khi Ä‘Ã£ hoÃ n thÃ nh
	useEffect(() => {
		if (!isProfileConfigured || profileConfigOpen) return;

		// Äáº©y má»™t sentinel entry vÃ o history Ä‘á»ƒ cÃ³ thá»ƒ intercept back
		window.history.pushState({ __onboardingDone: true }, '');

		const handlePopState = (event) => {
			if (event.state?.__onboardingDone) {
				// Re-push sentinel Ä‘á»ƒ ngÄƒn quay láº¡i â€” ngÆ°á»i dÃ¹ng Ä‘ang "back" vÃ o wizard
				window.history.pushState({ __onboardingDone: true }, '');
			}
		};
	...
	const handleSelectRoadmapPhase = useCallback((phaseId, options = {}) => {
		...
		// Giá»¯ nguyÃªn view hiá»‡n táº¡i khi chá»‰ auto-chá»n phase ná»n, trÃ¡nh bá»‹ kÃ©o khá»i quiz lÃºc reload.
	...
	const handleShareQuiz = useCallback(async (quiz) => {
		...
		showSuccess(
			shouldShare
				? t("workspace.quiz.sharedToCommunitySuccess", "ÄÃ£ chia sáº» quiz lÃªn cá»™ng Ä‘á»“ng.")
				: t("workspace.quiz.unsharedFromCommunitySuccess", "ÄÃ£ chuyá»ƒn quiz vá» private.")
		);
	}, [showSuccess, t]);
		...
		<div className={`h-screen flex flex-col overflow-hidden transition-colors duration-300 ${isDarkMode ? "bg-slate-950" : "bg-[#F7FBFF]"}`}>
					...
					<div className={`max-w-[1740px] mx-auto rounded-2xl border px-4 py-3 ${
						...
						<div className="flex flex-wrap items-center justify-between gap-3">
							...
							<div className="flex items-center gap-2">
								...
								>
									{mockTestGenerationState === "ready" ? "Má»Ÿ Mock test" : mockTestGenerationState === "pending" ? "Kiá»ƒm tra láº¡i ngay" : "Xem tráº¡ng thÃ¡i"}
								</Button>
							</div>
						</div>
					</div>
			...
			<div className="flex-1 min-h-0">
				<div ref={workspaceLayoutRef} className="max-w-[1740px] mx-auto px-4 py-4 h-full">
					{/* Layout workspace: bÃ¬nh thÆ°á»ng lÃ  3 cá»™t, mÃ n hÃ¬nh quÃ¡ nhá» thÃ¬ Ä‘Æ°a sources + studio xuá»‘ng dÆ°á»›i */}
