        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <select className={selectCls} value={status} onChange={(e) => setStatus(e.target.value)}>
              {["ACTIVE", "DRAFT", "COMPLETED", "INACTIVE"].map((s) => (
                <option key={s} value={s}>{t(`workspace.quiz.statusLabels.${s}`)}</option>
              ))}
            </select>
          </div>
          <label className={`flex items-center gap-2 cursor-pointer mt-4 ${fontClass}`}>
            ...
            <div key={q.questionId || `new-${qIdx}`} className={`rounded-lg border p-3 space-y-2 ${
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input type="number" className={inputCls} value={q.duration} onChange={(e) => updateQuestionField(qIdx, "duration", Number(e.target.value))} min={0} />
                </div>
                <div>
