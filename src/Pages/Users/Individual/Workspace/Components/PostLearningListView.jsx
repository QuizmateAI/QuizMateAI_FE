...
function PostLearningListView({ isDarkMode, onCreatePostLearning, onViewPostLearning, contextType = "WORKSPACE", contextId }) {
  ...
  // Tải danh sách post-learning: roadmaps → phases → quizzes theo PHASE context
  const fetchPostLearnings = useCallback(async () => {
    ...
    try {
      ...
      for (const rm of roadmaps) {
        ...
        } catch (e) { /* Bỏ qua roadmap không có phase */ }
      }
      setAllPhaseIds(allPhases.map(p => p.phaseId));

      // Tải quiz cho từng phase (contextType=PHASE)
      ...
      for (const ph of allPhases) {
        ...
        } catch (e) { /* Bỏ qua phase không có quiz */ }
      }
  ...
  // Kiểm tra tất cả phase đã có post-learning chưa
  ...
  return (
    <div className={`h-full flex flex-col ${fontClass}`}>
      ...
      {/* Warning khi tất cả phase đã có post-learning */}
