...
describe('ChallengeDetailView', () => {
  ...
  it('invites a reviewer immediately and accepts member.userID payloads', async () => {
    ...
    expect(screen.getByText('Reviewers are optional here. If invited, they can help check and clean up the match before you publish it.')).toBeInTheDocument();
  });
  ...
  it('hides the preview button when the leader joins the challenge and should only compose the quiz', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Compose match' })).toBeInTheDocument();
