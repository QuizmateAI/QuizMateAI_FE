...
export function getRenderableDocumentText(value, type) {
  const text = typeof value === 'string' ? value : String(value ?? '');
  const detected = classifyType(type);
  const trimmed = text.trim();
  const shouldNormalize = (detected === 'pdf' || detected === 'docx' || detected === 'unknown' || detected === 'text')
    && !hasMarkdownTableStructure(trimmed);
  return shouldNormalize ? normalizePdfExtractedText(trimmed) : trimmed;
}

function MarkdownBlock({ text, isDarkMode, fontClass }) {
  return (
    <ReactMarkdown
      remarkPlugins={[remarkGfm]}
      components={buildMarkdownComponents(isDarkMode, fontClass)}
    >
      {text}
    </ReactMarkdown>
  );
...
function HighlightedMarkdownBlock({
  text,
  highlightSpan,
  isDarkMode,
  fontClass,
  ...
  const segments = useMemo(
    () => getHighlightedContentSegments(text, highlightSpan),
    [text, highlightSpan],
  );
      ...
      {segments.map((segment, index) => {
        if (segment.highlight) {
          return (
            <mark
              key={`highlight-${index}`}
              ref={index === segments.findIndex((item) => item.highlight) ? highlightRef : null}
...
function buildMarkdownComponents(isDarkMode, fontClass) {
  return {
...
export function MaterialContentRenderer({
  ...
  highlightSpan = '',
  highlightRef = null,
  partialHighlightTitle = 'Tìm thấy một phần đoạn bằng chứng do nội dung nguồn khác định dạng.',
}) {
  const text = typeof value === 'string' ? value : String(value ?? '');
  ...
  const renderable = getRenderableDocumentText(trimmed, type);
  const trimmedHighlight = String(highlightSpan || '').trim();
    ...
    <div className={`material-md text-sm leading-relaxed ${isDarkMode ? 'text-slate-200' : 'text-slate-700'} ${fontClass}`}>
      {trimmedHighlight ? (
        <HighlightedMarkdownBlock
          text={renderable}
          highlightSpan={trimmedHighlight}
          isDarkMode={isDarkMode}
          fontClass={fontClass}
          highlightRef={highlightRef}
          partialTitle={partialHighlightTitle}
        />
      ) : (
        <MarkdownBlock
          text={renderable}
